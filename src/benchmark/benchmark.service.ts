...
export class BenchmarkService {
   ...
   */
  async getBenchmarks(query: BenchmarkQuery, limit: number = 50): Promise<FeeSlippageBenchmark[]> {
    let results = this.benchmarks.filter(benchmark => {
      if (query.bridgeName && benchmark.bridgeName !== query.bridgeName) return false;
      if (query.sourceChain && benchmark.sourceChain !== query.sourceChain) return false;
      if (query.destinationChain && benchmark.destinationChain !== query.destinationChain) return false;
   ...
   */
  async getLatestBenchmark(query: BenchmarkQuery): Promise<FeeSlippageBenchmark | null> {
  ...
  async processTransaction(transactionData: any): Promise<void> {
    ...
    const slippagePercent = slippage || this.calculateSlippage(
   ...
   */
  private calculateSlippage(inputAmount: number, outputAmount: number, feePercentage: number): number {
