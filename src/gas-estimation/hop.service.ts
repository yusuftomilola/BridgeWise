...
export class HopService {
   ...
   */
  normalizeRoute(rawRoute: any, request: RouteRequest): BridgeRoute {
    ...
    const estimatedReceived =
      rawRoute.estimatedReceived ?? rawRoute.amountOutMin ?? '0';
  ...
  normalizeFees(rawFees: any): HopFeeResponse {
    return {
      ...
      destinationTxFee:
        rawFees.destinationTxFee ?? rawFees.relayerFee ?? '2000',
